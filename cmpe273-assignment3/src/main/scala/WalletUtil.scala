...
object WalletUtil {
  ...
  def updateEtcdCounter(key: String, client: EtcdClient): Unit = {
    try {
      if (client.get(key) == null) {
        client.set(key, 1.toString)
      } else {
        var value: String = client.get(key).node.value
        println(value)
        client.set(key, (value.toInt + 1).toString)
      }
    } catch {
